...
- 本 skill 需要读取你的本地 OpenClaw 会话文件（包含你的对话历史）
- 本 skill 会将完整对话内容发送给 AI 代理进行提炼
- 本 skill 需要访问飞书 API，**必须完成用户 OAuth 授权**才能使用
- 所有飞书资源配置都通过环境变量指定，你必须自行配置你自己的飞书多维表格和文档
- 请确保你已经备份了重要数据，使用本技能产生的一切后果由使用者自行承担

### 权限范围说明

- **本地读取**：读取 `$SESSIONS_DIR` 目录下的会话文件（默认：`/root/.openclaw/agents/main/sessions/`）
- **AI 调用**：调用 OpenClaw agent 进行提炼（由 OpenClaw 控制，使用你配置的 AI 模型）
- **飞书写入**：调用飞书 API 将提炼结果写入你配置的多维表格和云文档
- **发送通知**：通过 OpenClaw CLI 发送通知到你的飞书
